...
try {
    ...
    Invoke-Checked $python @(
        'tests\stage8_course_guidance_check.py',
        '--guidance-module', (Join-Path $projectRoot 'future_aircraft_ws\src\multi_uav_mission\scripts\course_guidance.py'),
        '--course-spec', $spec
    )
    Invoke-Checked $python @(
    ...
    Invoke-Checked $python @(
        'tests\stage8_landing_disarm_check.py',
        '--executor-module', (Join-Path $projectRoot 'future_aircraft_ws\src\multi_uav_mission\scripts\mission_executor.py')
    )
    Invoke-Checked $python @(
