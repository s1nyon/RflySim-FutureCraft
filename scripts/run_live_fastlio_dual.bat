...
if defined STACK_MANIFEST (
  ...
  start "futureAircraftSim Stage 7 dual FAST-LIO" wsl -d %RFLYSIM_WSL_DISTRO% -e bash -lic "STACK_ID='%STACK_ID_ARG%' STACK_MANIFEST='!STACK_MANIFEST_WSL!' bash '%FUTURE_AIRCRAFT_SIM_WSL_DIR%/scripts/wsl/stage7_live_fastlio_dual.sh'" ^>nul 2^>^&1
) else (
  start "futureAircraftSim Stage 7 dual FAST-LIO" wsl -d %RFLYSIM_WSL_DISTRO% -e bash -lic "bash '%FUTURE_AIRCRAFT_SIM_WSL_DIR%/scripts/wsl/stage7_live_fastlio_dual.sh'" ^>nul 2^>^&1
)
exit /b 0
