...
set DRY_RUN=0
set SENSOR_MODE=lidar_only
:parse_args
if "%~1"=="" goto args_done
if /I "%~1"=="--dry-run" set DRY_RUN=1
if /I "%~1"=="--stack-id" set "STACK_ID_ARG=%~2"
if /I "%~1"=="--manifest" set "STACK_MANIFEST_ARG=%~2"
if /I "%~1"=="--sensor-mode" set "SENSOR_MODE=%~2"
shift & goto parse_args
:args_done
if /I not "%SENSOR_MODE%"=="lidar_only" if /I not "%SENSOR_MODE%"=="full" (
  echo [ERROR] --sensor-mode must be lidar_only or full
  exit /b 2
)
if "%DRY_RUN%"=="1" (
  echo [DRY-RUN] Stage 7 dual FAST-LIO live runner
  echo [DRY-RUN] sensor mode: %SENSOR_MODE%
...
if defined STACK_MANIFEST (
  ...
  start "futureAircraftSim Stage 7 dual FAST-LIO" wsl -d %RFLYSIM_WSL_DISTRO% -e bash -lic "STACK_ID='%STACK_ID_ARG%' STACK_MANIFEST='!STACK_MANIFEST_WSL!' STAGE7_SENSOR_MODE='%SENSOR_MODE%' bash '%FUTURE_AIRCRAFT_SIM_WSL_DIR%/scripts/wsl/stage7_live_fastlio_dual.sh'" ^>nul 2^>^&1
) else (
  start "futureAircraftSim Stage 7 dual FAST-LIO" wsl -d %RFLYSIM_WSL_DISTRO% -e bash -lic "STAGE7_SENSOR_MODE='%SENSOR_MODE%' bash '%FUTURE_AIRCRAFT_SIM_WSL_DIR%/scripts/wsl/stage7_live_fastlio_dual.sh'" ^>nul 2^>^&1
)
exit /b 0
