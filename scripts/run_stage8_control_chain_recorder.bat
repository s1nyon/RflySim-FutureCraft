...
if /I "%~1"=="--dry-run" (
  ...
  echo [DRY-RUN] 3. subscribe read-only: planner pos_cmd, setpoint_raw/local, slam odometry_raw, odometry/out, odometry/in, local_position/odom, mavros/state
  echo [DRY-RUN] 4. write stage8_control_chain.jsonl + stage8_control_chain_summary.json under $STAGE7_RUN_DIR
  echo [DRY-RUN] 5. no Publisher, ServiceProxy, arming, set_mode, or setpoint publishing
  exit /b 0
)
