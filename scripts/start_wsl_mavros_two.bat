...
if "%DRY_RUN%"=="1" (
  ...
  echo [DRY-RUN] Health gate: logs/live_stack/^<stack_id^>/health.json (GUI_READY/ROSCORE_READY/MAVROS_UAV1_CONNECTED/MAVROS_UAV2_CONNECTED/COURSE_READY)
  exit /b 0
)
...
if defined STACK_HEALTH_DIR_WSL (
  start "futureAircraftSim MAVROS two" wsl -d %RFLYSIM_WSL_DISTRO% -e bash -lic "STACK_HEALTH_DIR='%STACK_HEALTH_DIR_WSL%' STACK_ID='%STACK_ID%' bash '%FUTURE_AIRCRAFT_SIM_WSL_DIR%/scripts/wsl/stage2_two_mavros.sh'"
) else (
