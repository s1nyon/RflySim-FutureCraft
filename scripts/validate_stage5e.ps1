...
$requiredPaths = @(
    ...
    'scripts/wsl/stage5e_executor_sim_arm.sh',
...
if ($missing.Count -eq 0 -and $pythonRunner) {
    $outputDir = Join-Path $env:TEMP ("future_aircraft_stage5e_{0}" -f $PID)
    ...
    $traceFixturePath = Join-Path $ProjectRoot 'tests/fixtures/stage5e/expected_executor_trace.json'
    $eventsFixturePath = Join-Path $ProjectRoot 'tests/fixtures/stage5e/expected_mission_events.jsonl'
    $scoreFixturePath = Join-Path $ProjectRoot 'tests/fixtures/stage5e/expected_score_summary.json'
    ...
    elseif (-not (Test-Path -LiteralPath $planOutputPath)) {
        ...
        $executorOutput = Invoke-ContractPythonScript -Runner $pythonRunner -ScriptPath $executorScript -Arguments @('--plan', $planOutputPath, '--live-config', $liveConfigPath, '--backend', 'dry-run', '--allow-arm', '--simulation-only', '--events', $eventsOutputPath, '--trace', $traceOutputPath, '--score', $scoreOutputPath)
        if ($LASTEXITCODE -ne 0) {
    ...
    if ($contractErrors.Count -eq 0) {
        $stage5dScript = Join-Path $ProjectRoot 'scripts/validate_stage5d.ps1'
        $stage5dOutput = & powershell -ExecutionPolicy Bypass -File $stage5dScript -Quiet 2>&1
        if ($LASTEXITCODE -ne 0) {
            $contractErrors += "Stage 5D validation regression failed with exit code ${LASTEXITCODE}: $($stage5dOutput -join ' ')"
        }
    }
}


$launcherScript = Join-Path $ProjectRoot 'scripts/start_mission_executor_sim_arm.bat'
if (Test-Path -LiteralPath $launcherScript) {
    $launcherOutput = & cmd /c "`"$launcherScript`" --dry-run" 2>&1
    if ($LASTEXITCODE -ne 0) {
        $contractErrors += "scripts/start_mission_executor_sim_arm.bat --dry-run failed with exit code ${LASTEXITCODE}: $($launcherOutput -join ' ')"
    }
}
if ($missing.Count -gt 0 -or $contractErrors.Count -gt 0) {
    if (-not $Quiet) {
        Write-Host '[FAIL] Stage 5E simulation ARM executor validation failed.' -ForegroundColor Red
...
if (-not $Quiet) {
    Write-Host '[PASS] Stage 5E simulation ARM executor validation passed.' -ForegroundColor Green
}
exit 0
