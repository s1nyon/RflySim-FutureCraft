...
$q = [char]34
$new = 'wsl -d %RFLYSIM_WSL_DISTRO% -e bash -lic ' + $q + 'STACK_MANIFEST=''%STACK_MANIFEST_WSL%''; source ''%FUTURE_AIRCRAFT_SIM_WSL_DIR%/scripts/wsl/lifecycle_common.sh''; stack_register wsl $$ $$ wsl:px4_build_session sitl_multiple_run_rfly.sh created-by-stack-sitl-wrapper; echo Starting PX4 Build; cd %PSP_PATH_LINUX%/Firmware; ./BkFile/EnvOri.sh; export PATH=$HOME/ninja:$HOME/gcc-arm-none-eabi-7-2017-q4-major/bin:$PATH; make px4_sitl_default; ./Tools/sitl_multiple_run_rfly.sh %VehicleNum% %START_INDEX% %PX4SitlFrame%; echo FutureAircraftSim SITL running. Close this WSL process or window to stop.; tail -f /dev/null' + $q
