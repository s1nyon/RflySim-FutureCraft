...
def _send(api, dynamic: Dict[str, Any], elapsed: float, window_id: int) -> Dict[str, Any]:
    pose = pendulum_pose(dynamic, elapsed)
    ned = list(enu_to_ned(Vec3(*pose)))
    api.sendUE4PosNew(copterID=dynamic["id"], vehicleType=dynamic["vehicle_type"], PosE=ned, AngEuler=[0.0, 0.0, 0.0], windowID=window_id)
...
def run_samples(api, dynamic: Dict[str, Any], elapsed_values: Iterable[float], evidence_path: Path, window_id: int = -1) -> Dict[str, Any]:
    samples = [_send(api, dynamic, float(value), window_id) for value in elapsed_values]
...
def main() -> int:
    ...
    args = parser.parse_args(); dynamic = load_spec(args.spec)["dynamic_obstacle"]
    if args.dry_run:
